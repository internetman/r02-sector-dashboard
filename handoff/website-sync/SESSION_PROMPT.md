...
- 网站 `/m2-table` 是最近一次 i问财导入快照的关键字段表格页；更新导入文件后要重新生成 `m2-table-data.js`。
- M2 的 `m2-data.js` 是当前观察快照；更新时不能把“未确认 / 待计算”写成确定结论。
- M2 首页的候选股价格和日涨跌由 `/api/m2-watchlist` 同步，页面打开后每 5 分钟刷新；结构字段（Pivot 依据、收缩次数、底部时间、修正深度）仍需历史 OHLCV 扫描确认。
