...
- `/api/m2-history`：M2 候选股动态复权日K、MA50/150/200 与 VCP 收缩初筛 JSON

M2 的 `m2-data.js` 保存候选池和结构判断快照；候选股价格、日涨跌会从 `/api/m2-watchlist` 实时同步，页面打开后每 5 分钟刷新一次。若行情源失败，页面会明确显示“沿用上次”或“同步失败”，不会把旧价格伪装成实时价格。
...
M2 候选股行情自动刷新频率为 5 分钟，动态日K与 VCP 初筛刷新频率为 10 分钟；R02 雷达页面仍按原逻辑自动刷新 30 分钟；手动刷新按钮会强制重新加载页面和最新 API。M2 建议只做候选分层：`突破确认后考虑`、`不追当日大涨`、`等待平台/突破`、`暂不建议买入`；它不执行交易，最终买不买由使用者决定。
