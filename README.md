...
M2 选股快照建议在 A 股收盘后 15:30–16:00 生成一次，次日盘前或盘中只读展示；网页每 30 分钟检查一次是否已经上传了新快照。R02 雷达页面仍按原逻辑自动刷新 30 分钟；手动按钮会重新读取网页资源。Hermes 或本地 Session 可在收盘后执行 `bash handoff/website-sync/refresh-m2-site.sh`，一次完成抓取、分析和推送。M2 建议只做候选分层：`突破确认后考虑`、`不追当日大涨`、`等待平台/突破`、`暂不建议买入`；它不执行交易，最终买不买由使用者决定。
