...
- `/m2-snapshot.json`：网页正常读取的本地 Session 快照；打开网页不再等待六个历史行情请求

M2 的 `m2-data.js` 保存候选池和结构判断快照；`m2-snapshot.json` 由本地 Session 在收盘后统一抓取报价、日K、均线和 VCP 初筛结果，网页只负责读取和展示。快照超过 36 小时会标记为“已过期”，不会把旧数据伪装成当天数据。
...
M2 选股快照建议在 A 股收盘后 15:30–16:00 生成一次，次日盘前或盘中只读展示；网页每 30 分钟检查一次是否已经上传了新快照。R02 雷达页面仍按原逻辑自动刷新 30 分钟；手动按钮会重新读取网页资源。可运行 `python3 handoff/website-sync/capture-m2-snapshot.py` 生成快照，再执行 `handoff/website-sync/sync-to-vercel-repo.sh` 推送到网站。M2 建议只做候选分层：`突破确认后考虑`、`不追当日大涨`、`等待平台/突破`、`暂不建议买入`；它不执行交易，最终买不买由使用者决定。
